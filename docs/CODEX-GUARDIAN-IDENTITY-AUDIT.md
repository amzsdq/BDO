...
Parser audit also found that current recipe-detail regression fixtures use `랜덤 제품`, while live KR recipe 346 uses `추가 (무작위) 제품`. The collector must recognize the live heading before production-scale detail acquisition; otherwise a real random-output section can be misclassified as no-output.
