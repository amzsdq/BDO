...
test('missing, malformed, and unverified datasets stay visibly fail-closed', async ({ page }) => {
  ...
  await page.route(datasetUrl, (route) => route.fulfill({
    contentType: 'application/json',
    body: JSON.stringify({
      metadata: { generatedAt: '2026-09-26T00:00:00Z', sources: ['E2E unverified fixture'], supportedRegion: 'KR' },
  ...
  await page.reload()
  await expect(page.getByRole('status').filter({ hasText: '전체 레시피 대조 검증 전' })).toBeVisible()
  await expect(page.locator('.status-pill')).toHaveText('KR · 검증 중')
})
