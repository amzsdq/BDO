...
test('malformed planner import reports an error and preserves current storage', async ({ page }) => {
  await routeStateDataset(page); await seedPlannerState(page)
  ...
  await expect(page.getByRole('alert')).toBeVisible(); await expectSeededPlannerState(page)
...
test('unsupported planner export version reports an error and preserves current storage', async ({ page }) => {
  await routeStateDataset(page); await seedPlannerState(page)
  const before = await page.evaluate(() => ({ ...localStorage }))
  await page.getByLabel('계획 파일 가져오기').setInputFiles({ name: 'unsupported.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify({ version: 999, exportedAt: '2026-09-24T00:00:00Z', checklist: {}, inventory: {}, characterProfile: {}, planSession: {} })) })
  await expect(page.getByRole('alert')).toContainText('지원하지 않는 플래너 내보내기 파일입니다.'); await expectSeededPlannerState(page)
  await expect.poll(() => page.evaluate(() => ({ ...localStorage }))).toEqual(before)
})
...
test('planner import rolls storage back when a browser write fails mid-import', async ({ page }) => {
  await routeStateDataset(page); await seedPlannerState(page)
  const exportedPath = await exportPlanner(page)
  await page.getByLabel('준비할 재료 회분').fill('8'); await page.getByLabel('E2E 상태 재료 보유 수량').fill('4')
  const before = await page.evaluate(() => ({ ...localStorage }))
  await page.evaluate(() => {
    const original = Storage.prototype.setItem; let failed = false
    Storage.prototype.setItem = function (key: string, value: string) { if (!failed && key === 'bdo-planner:character-profile:v1') { failed = true; throw new Error('E2E quota') } return original.call(this, key, value) }
  })
  await page.getByLabel('계획 파일 가져오기').setInputFiles(exportedPath)
  await expect(page.getByRole('alert')).toContainText('E2E quota')
  await expect(page.getByLabel('준비할 재료 회분')).toHaveValue('8'); await expect(page.getByLabel('E2E 상태 재료 보유 수량')).toHaveValue('4')
  await expect.poll(() => page.evaluate(() => ({ ...localStorage }))).toEqual(before)
})

test('planner import surfaces an explicit error when browser rollback also fails', async ({ page }) => {
  await routeStateDataset(page); await seedPlannerState(page)
  const exportedPath = await exportPlanner(page)
  await page.getByLabel('준비할 재료 회분').fill('8')
  await page.evaluate(() => {
    const original = Storage.prototype.setItem; let calls = 0
    Storage.prototype.setItem = function (key: string, value: string) {
      calls += 1
      if (calls === 3) throw new Error('E2E import write failure')
      if (calls === 4) throw new Error('E2E rollback failure')
      return original.call(this, key, value)
    }
  })
  await page.getByLabel('계획 파일 가져오기').setInputFiles(exportedPath)
  await expect(page.getByRole('alert')).toContainText('이전 상태 복구에도 실패했습니다')
