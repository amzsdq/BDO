...
const dataset: RecipeDataset = {
  ...
  recipes: {
    meal: {
      id: 'meal', skill: 'cooking', outputItemId: 1,
      yield: { min: 1, max: 4, expected: 2.5 },
      variants: [{ id: 'default', inputs: [{ itemId: 2, count: 2 }, { itemId: 3, count: 1 }] }],
    },
    intermediate: {
      id: 'intermediate', skill: 'cooking', outputItemId: 2,
      yield: { min: 1, max: 1 },
      variants: [{ id: 'default', inputs: [{ itemId: 4, count: 3 }] }],
    },
  },
  recipesByOutput: { '1': ['meal'], '2': ['intermediate'] },
}

describe('planner', () => {
  it('defaults desired-output planning to guaranteed minimum yield', () => {
    expect(attemptsForTarget(dataset.recipes.meal, { recipeId: 'meal', mode: 'output', amount: 10 })).toBe(10)
  })

  it('supports explicit expected-yield planning', () => {
    expect(attemptsForTarget(dataset.recipes.meal, { recipeId: 'meal', mode: 'output', amount: 10, yieldPolicy: 'expected' })).toBe(4)
  })

  it('uses utensil attempts directly', () => {
    expect(attemptsForTarget(dataset.recipes.meal, { recipeId: 'meal', mode: 'attempts', amount: 150.2 })).toBe(151)
  })
  ...
  it('subtracts owned inventory from shortage without changing required quantity', () => {
    const plan = buildPlan(dataset, [{ recipeId: 'meal', mode: 'attempts', amount: 2 }], {
      craftIntermediateItemIds: new Set(), haveByItemId: { '3': 1 },
    })
  ...
  it('uses owned intermediate stock before recursively crafting the shortage', () => {
    const plan = buildPlan(dataset, [{ recipeId: 'meal', mode: 'attempts', amount: 3 }], {
      craftIntermediateItemIds: new Set([2]), haveByItemId: { '2': 4 },
    })
