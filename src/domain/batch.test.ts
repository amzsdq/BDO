...
describe('calculateBatchCapacity', () => {
  ...
  it('warns when a requested material batch exceeds available LT', () => {
    ...
    expect(result.warnings[0]).toContain('최대 10회분')
  })
  ...
  it('never produces negative available weight', () => { const result = calculateBatchCapacity(variant, items, { maxWeightLT: 100, reservedWeightLT: 150 }); expect(result.availableWeightLT).toBe(0); expect(result.maxServings).toBe(0) })
})
