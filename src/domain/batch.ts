...
export interface BatchLine {
  ...
  countToCarry: number
  weightPerItemLT: number
  weightToCarryLT: number
}

export interface BatchCapacity {
  availableWeightLT: number
  ingredientWeightPerServingLT?: number
  maxServings?: number
  lines: BatchLine[]
  unknownWeightItemIds: number[]
  warnings: string[]
}
...
/**
 * Computes exact starting ingredient load only. It intentionally does not
 * estimate output/byproduct peak weight; those require separately proven yield
 * distributions and must be presented as probabilistic estimates.
 */
export function calculateBatchCapacity(
  ...
  for (const input of variant.inputs) {
    ...
    if (!item || item.weightLT == null || !Number.isFinite(item.weightLT) || item.weightLT < 0) {
      unknownWeightItemIds.push(input.itemId)
      continue
    }
  ...
  if (unknownWeightItemIds.length) {
    ...
    return {
      availableWeightLT,
      lines: [],
      unknownWeightItemIds: [...new Set(unknownWeightItemIds)],
      warnings,
    }
  }
  ...
  const lines = variant.inputs.map((input) => {
    ...
    const countToCarry = input.count * maxServings
    return {
      itemId: input.itemId,
      countPerServing: input.count,
      countToCarry,
      weightPerItemLT,
      weightToCarryLT: weightPerItemLT * countToCarry,
    }
  })

  return { availableWeightLT, ingredientWeightPerServingLT, maxServings, lines, unknownWeightItemIds: [], warnings }
}
