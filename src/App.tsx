...
export function App({ bootstrap }: { bootstrap: ReadyBootstrap }) {
  ...
  const active = activeSessionTarget(dataset, session, Math.min(activeTargetIndex, Math.max(0, session.targets.length - 1)))
  ...
  return <main className="app-shell">
    ...
    {session.targets.length > 1 && <nav className="segmented" aria-label="제작 목표">{session.targets.map((target, index) => <button key={`${target.recipeId}-${index}`} className={index === active.index ? 'active' : ''} onClick={() => setActiveTargetIndex(index)}>{dataset.items[String(dataset.recipes[target.recipeId]?.outputItemId)]?.nameKo ?? `목표 ${index + 1}`}</button>)}</nav>}
