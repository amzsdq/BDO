...
export function App() {
  ...
  const batch = useMemo(() => { const variant = selectedRecipe?.variants[0]; if (!variant || profile.maxWeightLT == null) return null; return calculateBatchCapacity(variant, dataset.items, { maxWeightLT: profile.maxWeightLT, reservedWeightLT: profile.reservedWeightLT }) }, [dataset.items, profile.maxWeightLT, profile.reservedWeightLT, selectedRecipe])
  ...
  return <main className="app-shell">
    ...
    </aside><section className="panel checklist"><div className="section-heading"><span>02</span><div><strong>준비 체크리스트</strong><small>체크 상태는 이 기기에 자동 저장됩니다.</small></div></div><div className="material-list">{plan?.materials.map((material) => { const item = dataset.items[String(material.itemId)]; const done = checked[String(material.itemId)] ?? false; return <label className={`material-row ${done ? 'done' : ''}`} key={material.itemId}><input type="checkbox" checked={done} onChange={(e) => setChecked((current) => ({ ...current, [String(material.itemId)]: e.target.checked }))} /><div className="item-icon" aria-hidden="true">?</div><div className="material-name"><strong>{item?.nameKo ?? `아이템 #${material.itemId}`}</strong><small>{material.craftedIntermediate ? '중간재' : '재료'}</small></div><div className="quantity"><span>필요</span><strong>{material.required.toLocaleString()}</strong></div><div className="quantity missing"><span>부족</span><strong>{material.missing.toLocaleString()}</strong></div></label> })}</div></section></section>
  </main>
}
