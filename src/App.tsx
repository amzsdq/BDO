...
export function App() {
  ...
  useEffect(() => {
    void loadRuntimeDataset().then((loaded) => {
      ...
      const first = Object.values(loaded.dataset.recipes).find((recipe) => recipe.skill === skill)
      if (first) setRecipeId(first.id)
    })
  }, [])

  const results = useMemo(
    () => searchRecipes(dataset, query, { skill, limit: 8 }),
    [dataset, query, skill],
  )

  const selectedRecipe = dataset.recipes[recipeId]
  const selectedItem = selectedRecipe
    ? dataset.items[String(selectedRecipe.outputItemId)]
    : undefined
  ...
  const plan = useMemo(() => {
    ...
    return buildPlan(
      dataset,
      [{ recipeId: selectedRecipe.id, mode, amount }],
      ...
      <section className="planner-grid">
        <aside className="panel controls">
          ...
          <div className="search-wrap">
            <label className="field">
              ...
              <input
                ...
                placeholder={selectedItem?.nameKo ?? '예: 맥주'}
                aria-label="제작물 검색"
                autoComplete="off"
              />
            </label>
            {query.trim() && (
              <div className="search-results" role="listbox" aria-label="검색 결과">
                {results.length ? results.map(({ recipe, item }) => (
                  <button
                    key={recipe.id}
                    role="option"
                    aria-selected={recipe.id === recipeId}
                    onClick={() => { setRecipeId(recipe.id); setQuery('') }}
                  >
                    <span>{item.nameKo}</span><small>{recipe.skill === 'cooking' ? '요리' : '연금'}</small>
                  </button>
                )) : <p>일치하는 제작물이 없습니다.</p>}
              </div>
            )}
          ...
          <div className="selected-target">
            <small>선택한 제작물</small>
            <strong>{selectedItem?.nameKo ?? '선택 필요'}</strong>
          </div>
          ...
          <div className="summary-card">
            <span>예상 작업</span><strong>{plan?.crafts[0]?.attempts.toLocaleString() ?? '—'}회</strong>
            <small>변동 산출량은 검증된 범위 안에서 보수적으로 계산합니다.</small>
          </div>
        </aside>

        <section className="panel checklist">
          <div className="section-heading"><span>02</span><div><strong>준비 체크리스트</strong><small>필요량 · 보유량 · 부족량을 한 줄에서 확인합니다.</small></div></div>
