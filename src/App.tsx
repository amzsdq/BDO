...
import { readCharacterProfile, readChecklist, writeCharacterProfile, writeChecklist } from './data/storage'
...
export function App() {
  ...
  const plan = useMemo(() => selectedRecipe ? buildPlan(dataset, [{ recipeId: selectedRecipe.id, mode, amount }], { craftIntermediateItemIds: new Set() }) : null, [amount, dataset, mode, selectedRecipe])
  const batch = useMemo(() => {
    const variant = selectedRecipe?.variants[0]
    if (!variant || profile.maxWeightLT == null) return null
    return calculateBatchCapacity(variant, dataset.items, { maxWeightLT: profile.maxWeightLT, reservedWeightLT: profile.reservedWeightLT })
  }, [dataset.items, profile.maxWeightLT, profile.reservedWeightLT, selectedRecipe])
  const massCooking = useMemo(() => {
    if (skill !== 'cooking' || mode !== 'attempts' || profile.cookingMastery == null) return null
    return forecastCookingMaterialServings(amount, profile.cookingMastery) ?? null
  }, [amount, mode, profile.cookingMastery, skill])
  const alchemyMastery = useMemo(() => {
    if (skill !== 'alchemy' || profile.alchemyMastery == null) return null
    return verifiedAlchemyMasteryRow(profile.alchemyMastery) ?? null
  }, [profile.alchemyMastery, skill])
  ...
  return <main className="app-shell">
    ...
    <section className="planner-grid">
      <aside className="panel controls">
        ...
        {skill === 'cooking' && mode === 'attempts' && profile.cookingMastery != null && <div className="summary-card"><span>대량 요리 반영 재료 준비</span>{massCooking ? <><strong>기대 {Math.ceil(massCooking.expectedServings).toLocaleString()}회분</strong><small>최소 {massCooking.minimumServings.toLocaleString()} · 최대 {massCooking.maximumServings.toLocaleString()}회분 · 대량 요리 확률 {(massCooking.massCookingProbability * 100).toFixed(2)}%. 기대값은 확정 소비량이 아닙니다.</small></> : <><strong>검증된 숙련도 필요</strong><small>현재 숙련도는 검증된 공식 표의 정확한 구간값이 아니므로 확률을 임의 보간하지 않습니다.</small></>}</div>}
        ...
        {profile.maxWeightLT != null && <div className="summary-card batch-summary"><span>한 번에 준비 가능</span><strong>{batch?.maxServings != null ? `${batch.maxServings.toLocaleString()}회분` : '계산 보류'}</strong><small>{batch?.ingredientWeightPerServingLT != null ? `가용 ${batch.availableWeightLT.toLocaleString()} LT · 1회분 ${batch.ingredientWeightPerServingLT.toFixed(2)} LT` : batch?.warnings[0] ?? '레시피를 선택하세요.'}</small>{batch?.lines.length ? <ul className="carry-lines">{batch.lines.map((line) => <li key={line.itemId}><span>{dataset.items[String(line.itemId)]?.nameKo ?? `아이템 #${line.itemId}`}</span><strong>{line.countToCarry.toLocaleString()}개</strong></li>)}</ul> : null}</div>}
      </aside>
      <section className="panel checklist"><div className="section-heading"><span>02</span><div><strong>준비 체크리스트</strong><small>체크 상태는 이 기기에 자동 저장됩니다.</small></div></div><div className="material-list">{plan?.materials.map((material) => { const item = dataset.items[String(material.itemId)]; const done = checked[String(material.itemId)] ?? false; return <label className={`material-row ${done ? 'done' : ''}`} key={material.itemId}><input type="checkbox" checked={done} onChange={(e) => setChecked((current) => ({ ...current, [String(material.itemId)]: e.target.checked }))} /><div className="item-icon" aria-hidden="true">?</div><div className="material-name"><strong>{item?.nameKo ?? `아이템 #${material.itemId}`}</strong><small>{material.craftedIntermediate ? '중간재' : '재료'}</small></div><div className="quantity"><span>필요</span><strong>{material.required.toLocaleString()}</strong></div><div className="quantity missing"><span>부족</span><strong>{material.missing.toLocaleString()}</strong></div></label> })}</div></section>
    </section>
  </main>
}
