...
export function App({ bootstrap }: { bootstrap: ReadyBootstrap }) {
  ...
  const substitutionGroupIds = [...new Set((selectedVariant?.inputs ?? []).map((input) => input.substitutionGroupId).filter((id): id is string => Boolean(id)))]
  ...
  }, inventory, profile, { craftIntermediateItemIds, intermediateRecipeIdByItemId, selectedSubstitutionItemIdByGroupId: session.selectedSubstitutionItemIdByGroupId }) : { estimatedPreparation: false }, [amount, cookingPreparationPolicy, craftIntermediateItemIds, dataset, intermediateRecipeIdByItemId, inventory, mode, profile, selectedRecipe, selectedVariant, session.selectedSubstitutionItemIdByGroupId, yieldPolicy])
  ...
  function setSubstitution(groupId: string, rawItemId: string) {
    setSession((current) => {
      const selected = { ...current.selectedSubstitutionItemIdByGroupId }
      if (rawItemId) selected[groupId] = Number(rawItemId)
      else delete selected[groupId]
      return { ...current, selectedSubstitutionItemIdByGroupId: selected }
    })
  }
  ...
  return <main className="app-shell">
    ...
    <section className="planner-grid"><aside className="panel controls">
      ...
      {substitutionGroupIds.length > 0 && <div className="substitution-controls"><strong>대체 재료 선택</strong>{substitutionGroupIds.map((groupId) => { const group = dataset.substitutionGroups?.[groupId]; if (!group) return null; const canonicalInput = selectedVariant?.inputs.find((input) => input.substitutionGroupId === groupId); const canonicalName = canonicalInput ? dataset.items[String(canonicalInput.itemId)]?.nameKo ?? `#${canonicalInput.itemId}` : '대체 재료'; return <label className="field" key={groupId}><span>{canonicalName} · 대체 재료 {group.memberItemIds.length.toLocaleString()}종</span><select value={session.selectedSubstitutionItemIdByGroupId[groupId] ?? ''} onChange={(e) => setSubstitution(groupId, e.target.value)}><option value="">보유량 기준 자동 선택</option>{group.memberItemIds.map((id) => <option key={id} value={id}>{dataset.items[String(id)]?.nameKo ?? `아이템 #${id}`}</option>)}</select><small>직접 선택하면 준비 목록과 무게 계산에 같은 대체 비율을 적용합니다.</small></label> })}</div>}
      ...
      {activePlan.outputWeight && <div className="summary-card"><span>검증된 기본 산출물 무게</span><strong>{activePlan.outputWeight.minLT.toFixed(2)}~{activePlan.outputWeight.maxLT.toFixed(2)} LT</strong><small>현재 준비 회분의 기본 산출물 범위입니다.{activePlan.outputWeight.excludedRandomOutputItemIds.length ? ' 확률/상위 산출물은 발생 확률 근거가 없어 합산하지 않았습니다.' : ''}</small></div>}
