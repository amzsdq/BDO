...
export function App({ bootstrap }: { bootstrap: ReadyBootstrap }) {
  ...
  return <main className="app-shell">
    ...
    <section className="planner-grid"><aside className="panel controls">
      ...
      {substitutionGroupIds.length > 0 && <div className="substitution-controls"><strong>대체 재료 선택</strong>{substitutionGroupIds.map((groupId) => { const group = dataset.substitutionGroups?.[groupId]; if (!group) return null; const canonicalInput = selectedVariant?.inputs.find((input) => input.substitutionGroupId === groupId); const canonicalName = canonicalInput ? dataset.items[String(canonicalInput.itemId)]?.nameKo ?? `#${canonicalInput.itemId}` : '대체 재료'; return <SubstitutionSelector key={groupId} groupId={groupId} canonicalName={canonicalName} memberItemIds={group.memberItemIds} selectedItemId={session.selectedSubstitutionItemIdByGroupId[groupId]} itemName={(id) => dataset.items[String(id)]?.nameKo ?? `아이템 #${id}`} onSelect={(id) => setSubstitution(groupId, id == null ? '' : String(id))} /> })}</div>}
