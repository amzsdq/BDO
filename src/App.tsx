...
export function App() {
  ...
  const plan = useMemo(() => selectedRecipe ? buildPlan(dataset, [{ recipeId: selectedRecipe.id, mode, amount, variantId: selectedVariant?.id }], { craftIntermediateItemIds: new Set(), haveByItemId: inventory }) : null, [amount, dataset, inventory, mode, selectedRecipe, selectedVariant])
  const requestedServings = plan?.crafts[0]?.attempts
  const batch = useMemo(() => { if (!selectedVariant || profile.maxWeightLT == null) return null; return calculateBatchCapacity(selectedVariant, dataset.items, { maxWeightLT: profile.maxWeightLT, reservedWeightLT: profile.reservedWeightLT }, requestedServings) }, [dataset.items, profile.maxWeightLT, profile.reservedWeightLT, requestedServings, selectedVariant])
  ...
  return <main className="app-shell">
    ...
    <section className="planner-grid"><aside className="panel controls">
      ...
      {selectedRecipe && selectedRecipe.variants.length > 1 && <label className="field"><span>재료 조합</span><select value={selectedVariant?.id ?? ''} onChange={(e) => setVariantId(e.target.value)}>{selectedRecipe.variants.map((variant, index) => <option key={variant.id} value={variant.id}>조합 {index + 1} · {variant.inputs.map((input) => `${dataset.items[String(input.itemId)]?.nameKo ?? `#${input.itemId}`} ×${input.count}`).join(' + ')}</option>)}</select><small>대체 재료 조합을 직접 선택합니다. 선택한 조합은 준비 목록과 무게 계산에 동일하게 적용됩니다.</small></label>}
      ...
      {profile.maxWeightLT != null && <div className="summary-card batch-summary"><span>요청 작업 재료 적재</span><strong>{requestedServings != null && batch?.totalStartingIngredientWeightLT != null ? `${requestedServings.toLocaleString()}회분 · ${batch.totalStartingIngredientWeightLT.toFixed(2)} LT` : batch?.maxServings != null ? `최대 ${batch.maxServings.toLocaleString()}회분` : '계산 보류'}</strong><small>{batch?.ingredientWeightPerServingLT != null ? `가용 ${batch.availableWeightLT.toLocaleString()} LT · 1회분 ${batch.ingredientWeightPerServingLT.toFixed(2)} LT · 최대 적재 ${batch.maxServings?.toLocaleString() ?? '—'}회분` : batch?.warnings[0] ?? '레시피를 선택하세요.'}</small>{batch?.warnings.map((warning) => <small key={warning} role="status">{warning}</small>)}{batch?.lines.length ? <ul className="carry-lines">{batch.lines.map((line) => <li key={line.itemId}><span>{dataset.items[String(line.itemId)]?.nameKo ?? `아이템 #${line.itemId}`}</span><strong>{line.countToCarry.toLocaleString()}개</strong></li>)}</ul> : null}</div>}
