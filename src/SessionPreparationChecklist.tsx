...
export function SessionPreparationChecklist({ dataset, result, checked, setChecked, inventory, setInventory }: SessionPreparationChecklistProps) {
  ...
  return <section className="panel checklist">
    ...
    {materialKeys.length > 0 && <div className="checklist-progress" aria-label={`준비 진행률 ${preparedCount}/${materialKeys.length}`}><div><strong>{preparedCount.toLocaleString()} / {materialKeys.length.toLocaleString()} 준비 완료</strong><span>{progressPercent}%</span></div><progress value={preparedCount} max={materialKeys.length} aria-label="준비 진행률" /></div>}
