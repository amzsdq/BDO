...
describe('buildActivePlanView', () => {
  ...
  it('aggregates multiple substituted slots that resolve to the same carry item', () => {
    ...
    const result = buildActivePlanView(dataset, {
      recipeId: 'sample-cooking', variantId: 'default', mode: 'attempts', amount: 2, skill: 'cooking',
    }, {}, { maxWeightLT: 1000, reservedWeightLT: 0 }, {
    ...
    expect(result.batch?.lines).toEqual([
      expect.objectContaining({ itemId: substituteId, countPerServing: 9, countToCarry: 18 }),
    ])
