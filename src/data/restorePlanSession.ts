 ...
 * state from being treated as authoritative, distinguishes corrupt or forward-version
 * persisted data from a genuine first run, and rejects stale recipe/item references.
 */
export function restorePlanSession(
  ...
  if (read.status === 'unsupported-version') {
    return {
      restored: false,
      reason: 'unsupported-version',
      errors: [`저장된 계획 데이터 버전 v${read.persistedVersion}은(는) 현재 앱보다 새 버전입니다. 자동으로 덮어쓰지 않습니다.`],
    }
  }
