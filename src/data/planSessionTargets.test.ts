...
describe('plan session target editing', () => {
  it('edits one target without collapsing its sibling or support choices', () => {
    const next = replacePlanTarget(session, 0, {
      recipeId: 'cooking-a',
      variantId: 'a-1',
      mode: 'servings',
      amount: 99,
    ...
    expect(next.targets).toEqual([
      { recipeId: 'cooking-a', variantId: 'a-1', mode: 'servings', amount: 99 },
      session.targets[1],
    ])
  ...
  it('supports duplicate recipe targets because editing is index-based', () => {
    const withDuplicate = appendPlanTarget(session, {
      recipeId: 'cooking-a', variantId: 'a-1', mode: 'servings', amount: 5,
    })
    const next = replacePlanTarget(withDuplicate, 2, {
      recipeId: 'cooking-a', variantId: 'a-1', mode: 'servings', amount: 7,
    })

    expect(next.targets.map((target) => target.amount)).toEqual([10, 25, 7])
  })

  it('removes only the requested target', () => {
    const next = removePlanTarget(session, 0)
    expect(next.targets).toEqual([session.targets[1]])
    expect(next.variantIdByRecipeId).toEqual(session.variantIdByRecipeId)
  })
