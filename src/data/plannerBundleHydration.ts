...
import { hydratePrimaryPlan, type PrimaryPlanHydration } from './primaryPlanHydration'
...
export type PlannerBundleHydration =
  | { status: 'ready'; primaryPlan: PrimaryPlanHydration; checklist: ChecklistState; inventory: InventoryState; characterProfile: CharacterProfileState }
  | { status: 'recovery-required'; components: string[]; errors: string[] }

/**
 * Read the whole persisted planner bundle before App enables any write-through.
 * A corrupt support-state component or non-restorable plan session keeps the entire
 * bundle in recovery so default/sample state cannot erase the original browser data.
 */
export function hydratePlannerBundle(
  ...
  const primaryPlan = hydratePrimaryPlan(dataset, storage)
  ...
  if (primaryPlan.status === 'recovery-required') {
    components.push('plan-session')
    errors.push(...primaryPlan.errors)
  }
  if (components.length) {
    if (components.some((component) => component !== 'plan-session')) errors.unshift('저장된 지원 상태 일부를 읽을 수 없습니다. 자동으로 덮어쓰지 않습니다.')
    return { status: 'recovery-required', components, errors }
  }
