...
function persistedEmpty(): PlanSessionState { return { version: 1, targets: [], craftIntermediateItemIds: [], intermediateRecipeIdByItemId: {}, variantIdByRecipeId: {}, selectedSubstitutionItemIdByGroupId: {} } }
describe('hydratePrimaryPlan', () => {
  ...
  it('preserves a valid intentionally empty persisted plan and its durable choices', () => { const empty = persistedEmpty(); empty.craftIntermediateItemIds = [900001]; const result = hydratePrimaryPlan(sampleDataset, storage(JSON.stringify(empty))); expect(result.status).toBe('restored-empty'); if (result.status === 'restored-empty') expect(result.session.craftIntermediateItemIds).toEqual([900001]) })
