...
describe('hydratePrimaryPlan', () => {
  ...
  it('preserves a valid intentionally empty persisted plan and its durable choices', () => { const empty = persistedEmpty(); empty.craftIntermediateItemIds = [200]; const result = hydratePrimaryPlan(sampleDataset, storage(JSON.stringify(empty))); expect(result.status).toBe('restored-empty'); if (result.status === 'restored-empty') expect(result.session.craftIntermediateItemIds).toEqual([200]) })
