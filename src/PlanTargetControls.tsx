...
const YIELD_POLICIES: Array<{ value: YieldPolicy; label: string }> = [
  { value: 'minimum', label: '최소 산출량' },
  { value: 'expected', label: '기대 산출량' },
  { value: 'maximum', label: '최대 산출량' },
]
...
export function PlanTargetControls(props: PlanTargetControlsProps) {
  const { skill, mode, amount, yieldPolicy, cookingPreparationPolicy } = props
    ...
    <>
      ...
      {mode === 'output' ? (
        <label className="field plan-policy-field">
          결과물 산출 기준
          <select value={yieldPolicy} onChange={(event) => props.onYieldPolicyChange(event.target.value as YieldPolicy)}>
            {YIELD_POLICIES.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}
          </select>
          <small>선택한 산출량 기준으로 목표 수량을 제작 횟수와 재료 수량으로 환산합니다.</small>
        </label>
      ) : null}
      {mode === 'durability' && skill === 'cooking' ? (
        <label className="field plan-policy-field">
          ...
          <select value={cookingPreparationPolicy} onChange={(event) => props.onCookingPreparationPolicyChange(event.target.value as CookingPreparationPolicy)}>
            {PREPARATION_POLICIES.map((entry) => <option key={entry.value} value={entry.value}>{entry.label}</option>)}
          </select>
