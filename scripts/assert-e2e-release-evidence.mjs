...
if (!gitSha.test(manifest.mainCommit ?? '')) fail('mainCommit must be an exact 40-char git SHA');
for (const key of ['datasetFingerprint', 'reconciliationFingerprint', 'masteryEvidenceFingerprint']) {
  if (!nonEmpty(manifest[key])) fail(`${key} is required`);
}
if (!sha256.test(manifest.masterySha256 ?? '')) fail('masterySha256 must be a 64-char SHA-256');
if (!nonEmpty(manifest.reconciliationTimestamp)) fail('reconciliationTimestamp is required');
if (Number.isNaN(Date.parse(manifest.reconciliationTimestamp))) fail('reconciliationTimestamp must be ISO-8601 parseable');
if (!Array.isArray(manifest.browsers) || manifest.browsers.length === 0 || manifest.browsers.some((x) => !nonEmpty(x))) fail('at least one browser is required');
...
for (const scenario of manifest.scenarios) {
  if (!scenario || !nonEmpty(scenario.id)) fail('scenario id is required');
...
for (const id of required) {
  ...
  if (!nonEmpty(scenario.evidence)) fail(`${id} evidence pointer is required`);
}
