...
$ServiceIni = Join-Path $ResolvedGameDir "service.ini"
if (-not (Test-Path $ServiceIni -PathType Leaf)) { throw "KR client validation failed: service.ini is missing" }
$ServiceIniText = Get-Content $ServiceIni -Raw
if ($ServiceIniText -notmatch '(?im)^\s*TYPE\s*=\s*KR\s*
...
) { throw "KR client validation failed: service.ini must declare TYPE=KR" }
