...
if (args.length !== 5 || args.some((value) => !value || value.startsWith('--'))) {
  fail('usage: node scripts/assert-production-release.mjs <dataset.json> <reconciliation-report.json> <codex-catalog.json> <mastery-evidence.json> <mastery.json>')
}
...
const existingGate = spawnSync(process.execPath, ['scripts/assert-release-readiness.mjs', ...args], {
  cwd: process.cwd(),
  encoding: 'utf8',
})
if (existingGate.status !== 0) {
  process.stderr.write(existingGate.stderr || existingGate.stdout || 'release blocked: existing readiness gate failed\n')
  process.exit(1)
}
