...
describe('Codex reconciliation', () => {
  it('earns ZERO_UNEXPLAINED_DIFF when canonical item ids and counts agree', () => {
    const { exitCode, report } = run(dataset, { recipes: [{ recipeId: 999, skill: 'cooking', outputItemId: 10, titleKo: '결과', ingredients: [{ itemId: 20, name: '다른 표기여도 ID가 우선', count: 2 }] }] })
  ...
  it('reports a deterministic signature mismatch when canonical ingredient counts differ', () => {
    ...
    expect(exitCode).toBe(2); expect(report.status).toBe('INCOMPLETE_REVIEW'); expect(report.unresolved[0].kind).toBe('SIGNATURE_MISMATCH')
  })
  it('keeps unavailable Codex recipes out of live completeness diffs', () => {
    const { exitCode, report } = run(dataset, { recipes: [{ recipeId: 999, skill: 'cooking', outputItemId: 10, titleKo: '결과', ingredients: [{ itemId: 20, count: 2 }] }, { recipeId: 1000, skill: 'alchemy', outputItemId: 77, titleKo: '퇴역', available: false, ingredients: [] }] })
