...
const evidence = { source: 'reviewed KR recipe yield evidence', entries: [{ recipeId: 'cooking:1', min: 1, expected: 2.5, max: 4, sourceUrl: 'https://example.com/evidence/1' }] }
...
describe('applyYieldEvidence', () => {
  it('replaces unknown yield with bounded reviewed evidence and invalidates stale fingerprint', () => {
    ...
    expect(result.recipes['cooking:1'].yield).toEqual({ min: 1, expected: 2.5, max: 4, provenance: 'https://example.com/evidence/1' })
  ...
  it('fails closed on unknown/duplicate recipes, invalid bounds, or non-https provenance', () => {
    ...
    expect(() => applyYieldEvidence(dataset, { entries: [{ ...evidence.entries[0], sourceUrl: 'http://example.com' }] })).toThrow(/https URL/)
  })
})
