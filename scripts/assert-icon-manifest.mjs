...
export function assertIconManifest(dataset, manifest, iconRoot) {
 ...
 for(const r of rows){if(r.path!==`icons/${r.itemId}.webp`||!Number.isInteger(r.bytes)||r.bytes<=0||!/^[a-f0-9]{64}$/.test(r.sha256)) throw new Error(`icon manifest row invalid for item ${r.itemId}`);const file=path.join(iconRoot,`${r.itemId}.webp`);if(!fs.existsSync(file)) throw new Error(`icon file missing for item ${r.itemId}`);const bytes=fs.readFileSync(file);if(bytes.length<12||bytes.toString('ascii',0,4)!=='RIFF'||bytes.toString('ascii',8,12)!=='WEBP'||bytes.readUInt32LE(4)+8!==bytes.length) throw new Error(`icon file is not valid RIFF/WEBP for item ${r.itemId}`);if(bytes.length!==r.bytes||digest(bytes)!==r.sha256) throw new Error(`icon file hash mismatch for item ${r.itemId}`)}
