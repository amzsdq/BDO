...
describe('adopt-viewer-client-snapshot', () => {
  it('binds reviewed viewer output to exact KR client bytes and rejects relabeling', () => {
    ...
    fs.writeFileSync(path.join(viewer,'.icon_provenance'),'stale')
    const staleIcons=spawnSync(process.execPath,['scripts/adopt-viewer-client-snapshot.mjs',viewer,game,path.join(root,'stale-icons')],{cwd:process.cwd(),encoding:'utf8'})
    expect(staleIcons.status).not.toBe(0); expect(staleIcons.stderr).toMatch(/icon provenance/)
    fs.writeFileSync(path.join(viewer,'.icon_provenance'),hash([Buffer.from(`${fp.slice(0,16)}|icons|3`)]).slice(0,16))
