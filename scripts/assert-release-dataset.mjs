...
const actualReconciliationSha256 = sha256(reconciliationBytes)
if (metadata.reconciliationSha256 !== actualReconciliationSha256) fail(`reconciliation artifact does not match promoted evidence: recorded=${metadata.reconciliationSha256 || 'missing'} actual=${actualReconciliationSha256}`)
if (metadata.reconciliationGeneratedAt !== reconciliation.generatedAt) fail('reconciliation generatedAt does not match promoted evidence')
...
console.log(JSON.stringify({ ok: true, status: metadata.status, counts: actualCounts, items: Object.keys(items).length, fingerprint: metadata.fingerprint, reconciliation: reconciliation.status, reconciliationSha256: actualReconciliationSha256, codexCatalogPages: completeCatalogPages, codexCatalogSha256: actualCatalogSha256 }))
